...
impl<I1, I2, S, F> Sans<I1> for MapInput<S, F>
where
    S: Sans<I2>,
    F: FnMut(I1) -> I2,
{
    ...
    fn next(&mut self, input: I1) -> Step<Self::Output, Self::Return> {
        let i2 = (self.f)(input);
        self.coro.next(i2)
    }
}
...
/// assert_eq!(mapped.next(5).unwrap_yielded(), "10");
/// ```
pub fn map_yield<I, O1, O2, S, F>(f: F, coro: S) -> MapYield<S, F>
where
    S: Sans<I, Output = O1>,
    F: FnMut(O1) -> O2,
{
...
impl<I, O1, O2, S, F> Sans<I> for MapYield<S, F>
where
    S: Sans<I, Output = O1>,
    F: FnMut(O1) -> O2,
{
...
impl<I, D1, D2, S, F> Sans<I> for MapReturn<S, F>
where
    S: Sans<I, Return = D1>,
    F: FnMut(D1) -> D2,
{
